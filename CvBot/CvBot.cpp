// CvBot.cpp : Defines the entry point for the console application.
//
...
void startmotorA(DBL v)
{
	(olDaVoltsToCode(min0, max0, gain, resolution, encoding, v, &iValue));  //To Stop Motor
	//(olDaPutSingleValue(hDA, iValue, channel[0], gain));
	(olDaPutSingleValue(hDA, iValue, channel[1], gain));
}
void startmotorB(DBL v)
{
	(olDaVoltsToCode(min0, max0, gain, resolution, encoding, v, &iValue));  //To Stop Motor
	(olDaPutSingleValue(hDA, iValue, channel[0], gain));
	//(olDaPutSingleValue(hDA, iValue, channel[1], gain));
...
void trackFilteredObject(FILE *fp, int &x, int &y, Mat threshold, Mat &cameraFeed) {
		...
		if (numObjects<MAX_NUM_OBJECTS) {
			for (int index = 0; index >= 0; index = hierarchy[index][0]) {
				...
				//	fprintf(fp, "%d\t%d\n", x, y);
					...
					if (area < 15000.00 && x<420 && x>160) {
						startmotor(2);
					}

					else if (area < 15000.00 && x>=420) {
						startmotorB(2.2);
						startmotorA(2.7);


					}
					else if (area < 15000.00 && x <= 160) {
						startmotorA(2.2);
						startmotorB(2.7);
					}
					else if (area > 15000.00)
					{
						startmotor(2.5);
					}
					else {
...
int main(int argc, char* argv[])
{
	...
	while (1) {
		...
		inRange(HSV, Scalar(0,138,107), Scalar(17,255,255), threshold);
		...
		//filtered object
		if (trackObjects)
			trackFilteredObject(fp, x, y, threshold, cameraFeed);
		...
		if (waitKey(30) == 27) break;//press esc to exit from the program
	}
